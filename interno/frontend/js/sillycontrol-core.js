...
import { ctrl, id, esc, toast, isEnInput, abbreviateName } from './control/control-shared.js';
// Módulos de responsabilidad (se adjuntan a `ctrl` como efecto secundario).
import './control/control-modos.js';
import './control/control-canva.js';
import './control/control-audio.js';
...
function initTabs() {
  ...
  document.querySelectorAll('.tab-panel').forEach(p => { p.setAttribute('role', 'tabpanel'); p.setAttribute('tabindex', '0'); });
  tabs.forEach(btn => {
    if (btn.id === 'displayPreviewToggle') {
      btn.addEventListener('click', () => {
        ctrl.toggleDisplayPreview();
        btn.setAttribute('aria-checked', String(!JSON.parse(btn.getAttribute('aria-checked') || 'false')));
      });
      return;
    }
...
// Re-exporta la superficie pública para consumidores de módulos
// (la API sigue expuesta globalmente vía window.ctrl, definido en
// control-shared.js). Los métodos se adjuntan a `ctrl` desde este core
// y desde los módulos de responsabilidad en js/control/.
export { ctrl };
